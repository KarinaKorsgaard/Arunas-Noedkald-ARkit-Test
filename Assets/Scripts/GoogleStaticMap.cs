...
public class GoogleStaticMap : MonoBehaviour {
	...
	public int horizontalSize = 640;
	public int verticalSize = 640;
	...
	public void initialize () {
		...
		addMarker (55.0205536f, 12.2885151f);
	}
	...
	public void UpdateZoom(){
		if (Input.touchCount == 2)
		{
			...
			zoomf += deltaMagnitudeDiff * -0.0005f;
		...
		int tempZ = mapZoom;
		mapZoom = (int) Mathf.Floor (zoomf);
		mapZoom = Mathf.Max (mapZoom, 15);
		//	DebugConsole.Log ("zoom:" + zoomf);
		if(mapZoom!=tempZ)DrawMap();
	}

	public bool addMarker(float lat, float lon){
		...
		foreach (var i in markers) {
			...
			foreach (var loc in i.locations) {
				bool diffY = Mathf.Abs (loc.latitude - lat)  < 0.0001;
				bool diffX = Mathf.Abs (loc.longitude - lon) < 0.0001;
				tooMany++;

				if (( diffY && diffX ) || tooMany > 8) {
					exists = true;
					//print ("exists" + i + " loc "+loc + " lat "+ lat + " lon "+lon);
					//print ("is here" + i + " lat "+ loc.latitude + " lon "+loc.longitude);
					break;
		...
		if (!exists) {
			...
			m.size = new GoogleMapMarkerSize ();
			m.size = GoogleMapMarkerSize.Small;

			m.color = new GoogleMapColor ();
			m.color = GoogleMapColor.green;
			m.label = "center";

			m.locations = new List<GoogleMapLocation> ();

			GoogleMapLocation l = new GoogleMapLocation ();
			l.address = "";
			l.latitude = lat;
			l.longitude = lon;
	...
	public void setCenterLocation(float lat, float lon){
		markers [0].locations [0].latitude = lat;
		markers [0].locations [0].longitude = lon;

		_centerLatLon.lat_d = lat;
		_centerLatLon.lon_d = lon;

	//	DebugConsole.Log (lat + " " + lon);
	}
	...
	IEnumerator DrawMapCoroutine ()
	{
		if (MAX_MAP_REQUEST_NUM != 0 && countOfMapRequests < MAX_MAP_REQUEST_NUM) {
			...
			string parameters = "&zoom=" + mapZoom.ToString ();
			//parameters += "&size=" + horizontalSize.ToString () + "x" + verticalSize.ToString ();
			parameters += "&size=" + WWW.UnEscapeURL (string.Format ("{0}x{0}", horizontalSize));
...
//			}
			...
			string url = "";
			url = baseURL + location + parameters+ qs + style + (_apiKey == "" ? "" : "&key=" + _apiKey);
			findCorners ();
